const sleepAnimalGroups = [
  {
    key: "insomnia-and-fragmentation",
    name: "Insomnia and Fragmentation",
    intro: "Sleep onset problems, middle-of-the-night waking, light sleep, and rumination-driven nights.",
    items: [
  ...
  {
    key: "circadian-and-schedule",
    name: "Circadian and Schedule",
    intro: "Sleep timing, chronotype, jet lag, shift work, and unstable daily rhythm.",
    items: [
  ...
  {
    key: "quantity-and-life-constraints",
    name: "Quantity and Life Constraints",
    intro: "Sleep debt, overload, caregiving, commute, and real-life limits that compress recovery.",
    items: [
  ...
  {
    key: "airway-environment-and-physical",
    name: "Airway, Environment, and Physical Disruption",
    intro: "Snoring, airway strain, pain, heat, noise, restless legs, and physical disruption during the night.",
    items: [
  ...
  {
    key: "nonrestorative-and-optimization",
    name: "Nonrestorative Sleep and Optimization",
    intro: "Sleep quality, recovery, performance, and the sleepers who want refinement instead of rescue.",
    items: [
  ...
  {
    key: "neuropsych-and-complex",
    name: "Neuropsych and Complex Sleep Profiles",
    intro: "Dream-related sleep, slow starts, sleep-transition issues, and stress-amplified complex nights.",
    items: [
  ...
  {
    key: "special-overlap-a",
    name: "Special Overlap Profiles A",
    intro: "When two important sleep problems overlap and one-dimensional advice is not enough.",
    items: [
  ...
  {
    key: "special-overlap-b",
    name: "Special Overlap Profiles B",
    intro: "Seasonal, emotional, high-sleep-need, travel-heavy, and recovery-fragmented profiles.",
    items: [
  ...
  },
  {
    ...
    intro: "The general starting point when the pattern is flexible or still emerging.",
    items: [
...
const searchInput = document.querySelector("#animal-search");
const animalGroupsContainer = document.querySelector("#animal-groups");
...
function renderGroups(query = "") {
  ...
  const groupsMarkup = sleepAnimalGroups
    .map((group, groupIndex) => {
      const matches = group.items.filter((item) => {
        const haystack = `${item.animal} ${item.title} ${item.hook} ${group.name} ${group.intro}`.toLowerCase();
      ...
      return `
        <details class="animal-group" ${normalized || groupIndex === 0 ? "open" : ""}>
          <summary>
            <span>${group.name}</span>
            <span>${matches.length} profile${matches.length === 1 ? "" : "s"}</span>
          </summary>
          <div class="animal-group-grid">
            ${matches
              .map(
                (item) => `
                  <article class="taxonomy-card">
                    <span class="taxonomy-animal-name">${item.animal}</span>
                    <h3>${item.title}</h3>
                    <p>${item.hook}</p>
                  </article>
                `
  ...
  animalGroupsContainer.innerHTML =
    groupsMarkup ||
    `<div class="taxonomy-empty">No sleep animals matched that search. Try keywords like insomnia, schedule, snoring, shift, stress, caregiver, or jet lag.</div>`;
}
...
document
  .querySelectorAll('img[data-make-transparent="true"]')
  .forEach((img) => makeWhiteTransparent(img));
